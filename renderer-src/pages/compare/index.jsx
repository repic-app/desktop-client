...
const calcZoomScale = (currentScale, isZoomOut) => {
  ...
  const stepValue = isZoomOut ? (currentScale > 100 ? 50 : 10) : currentScale >= 100 ? 50 : 10

  const nextScale = isZoomOut
    ? currentScale - stepValue < 50
      ? 50
      : currentScale - stepValue
    : currentScale + stepValue > 1600
    ? 1600
    : currentScale + stepValue
...
export default React.memo(() => {
  ...
  const applyZoom = (
    ...
    const nextTranslateX = resetTranslate
      ? 0
      : viewState.translateX - widthDiff * (isMouseEvent ? (originX - left) / width : originX)
    const nextTranslateY = resetTranslate
      ? 0
      : viewState.translateY - heightDiff * (isMouseEvent ? (originY - top) / height : originY)
  ...
  const handleMouseWheel = (event) => {
    !mouseDragging &&
      applyZoom(
        calcZoomScale(viewState.scale, event.deltaY > 0),
        event.pageX,
        event.pageY,
        true,
        false
